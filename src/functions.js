...
const safeNeighboardood = (board, row, column) => {
    const safes = (result, neighbor) => result && !neighbor.mined
    return getNeighbors(board, row, column).reduce(safes, true)
}

const onOpenField = (board, row, column) => {
    const field = board[row][column]
    if (!field.opened) {
        field.opened = true
        if (field.mined) {
            field.exploded = true
        } else if (safeNeighboardood(board, row, column)) {
            getNeighbors(board, row, column)
                .forEach(n => onOpenField(board, n.row, n.column))
        } else {
